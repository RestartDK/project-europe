...
function inferDomains(person: PDLPerson): string[] {
  ...
  if (
    all.includes("data") ||
    all.includes("machine learning") ||
    all.includes("ml")
  )
    domains.push("data")
  if (all.includes("security") || all.includes("crypto"))
    domains.push("security")
  if (all.includes("mobile") || all.includes("ios") || all.includes("android"))
    domains.push("mobile")
  if (all.includes("frontend") || all.includes("react") || all.includes("ui"))
    domains.push("frontend")
  if (all.includes("backend") || all.includes("api") || all.includes("server"))
    domains.push("backend")
  if (all.includes("full stack") || all.includes("fullstack"))
...
function roleKeywordsFromPdl(person: PDLPerson): string[] {
  ...
  if (person.job_title)
    kw.push(...person.job_title.toLowerCase().split(/\s+/).slice(0, 4))
  return [...new Set(kw)]
}
...
export const runPdlSearch = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    } | null = await ctx.runQuery(internal.pdlSearch.getSearchRequestCriteria, {
      requestId: args.requestId,
    })
    ...
    for (const { person, pdlScore } of ranked) {
      const linkedinUrl = optionalString(person.linkedin_url)
      if (!linkedinUrl) continue
      const slug = slugFromLinkedin(linkedinUrl)

      const personId: Id<"people"> = await ctx.runMutation(
        internal.pdlSearch.upsertPerson,
        {
          linkedinUrl,
          fullName: person.full_name,
          headline: optionalString(person.job_title),
          location:
            [person.location_locality, person.location_country]
              .filter(Boolean)
              .join(", ") || undefined,
          currentCompany: optionalString(person.job_company_name),
          stacks: stringList(person.skills).slice(0, 20),
          domains: inferDomains(person),
          yearsExperience: optionalNumber(person.inferred_years_experience),
          socialGithub: optionalString(person.github_url),
          pdlId: person.id,
          pdlData: person,
        }
      )
