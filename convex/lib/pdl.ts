...
export async function normalise(
  ...
  return {
    jobTitleRole:
      typeof raw.jobTitleRole === "string" ? raw.jobTitleRole : null,
    jobTitleSubRoles: Array.isArray(raw.jobTitleSubRoles)
      ? raw.jobTitleSubRoles
      : [],
    jobTitleLevels: Array.isArray(raw.jobTitleLevels) ? raw.jobTitleLevels : [],
    requiredSkills: Array.isArray(raw.requiredSkills)
      ? raw.requiredSkills.slice(0, 5)
      : [],
    preferredSkills: Array.isArray(raw.preferredSkills)
      ? raw.preferredSkills.slice(0, 8)
      : [],
    minYearsExperience:
...
export function buildQuery(schema: PDLSearchSchema): object {
  ...
  if (schema.jobTitleRole)
    must.push({ term: { job_title_role: schema.jobTitleRole } })
  if (schema.countries.length > 0)
    must.push({ terms: { location_country: schema.countries } })
  if (schema.minYearsExperience !== null && schema.minYearsExperience > 0) {
    must.push({
      range: {
        inferred_years_experience: { gte: schema.minYearsExperience - 1 },
      },
    })
  }

  for (const s of schema.jobTitleSubRoles)
    should.push({ term: { job_title_sub_role: s } })
  if (schema.jobTitleLevels.length > 0)
    should.push({ terms: { job_title_levels: schema.jobTitleLevels } })
  for (const s of schema.requiredSkills)
    should.push({ term: { skills: s.toLowerCase() } })
  for (const s of schema.preferredSkills)
    should.push({ term: { skills: s.toLowerCase() } })
  for (const l of schema.localities)
    should.push({ match: { location_name: l } })
...
export function scorePdlPerson(
  person: PDLPerson,
  schema: PDLSearchSchema
): number {
  let s = 0
  ...
  if (required.length > 0 || preferred.length > 0) {
    ...
    const pHits = preferred.filter((x) =>
      cSkills.some((c) => c === x || c.includes(x) || x.includes(c))
    ).length
    s +=
      (required.length > 0 ? (rHits / required.length) * 35 : 17) +
  ...
  s +=
    schema.jobTitleLevels.length > 0
      ? schema.jobTitleLevels.some((l) => levels.includes(l))
        ? 30
        : 0
      : 15

  s +=
    schema.jobTitleSubRoles.length > 0
      ? schema.jobTitleSubRoles.includes(person.job_title_sub_role ?? "")
        ? 20
        : 5
      : 10
