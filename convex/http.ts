  ...
  handler: httpAction(async (ctx, request) => {
    ...
    try {
      ...
      const dossiers = await Promise.all(
        search.results.map(
          async (result: { scoreId: Id<"candidateScores"> }) =>
      ...
      const payload = {
        ...
        enrichmentSummary: search.enrichmentSummary,
      ...
      console.log("[http:/api/rank] response", {
        ...
        topPreview: payload.results
          .slice(0, 5)
          .map((r: { rank: number; fullName: string; finalScore: number }) => ({
            rank: r.rank,
            name: r.fullName,
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ error: "Invalid JSON", raw: rawText }),
        {
          status: 400,
    ...
    if (
      ...
      console.log(
        "[clay-webhook] validation failed — missing or invalid convex_candidate_id",
        { body: JSON.stringify(body) }
      )
      return new Response(
        JSON.stringify({
          error: "Missing required field: convex_candidate_id",
          received: body,
        }),
    ...
    try {
      await ctx.runMutation(internal.candidates.updateFromClay, {
        ...
        stacks: Array.isArray(data.stacks)
          ? (data.stacks as string[])
          : undefined,
      })
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err)
      console.log("[clay-webhook] mutation error:", {
        candidateId,
        error: message,
      })
      if (
        ...
        return new Response(
          JSON.stringify({ error: "Candidate not found", detail: message }),
          {
            status: 404,
            headers: { "Content-Type": "application/json" },
          }
